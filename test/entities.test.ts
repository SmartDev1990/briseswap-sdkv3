...
const WNATIVE = _WNATIVE[ChainId.BSC_TESTNET]
    ...
    describe(`decimals permutation: ${decimals}`, () => {
      ...
      it('Pair', () => {
        pairs = [
          ...
          new Pair(
            new TokenAmount(tokens[2], decimalize(1, tokens[2].decimals)),
            new TokenAmount(WNATIVE, decimalize(1234, WNATIVE.decimals))
          )
        ]
      })
      ...
      it('Route', () => {
        ...
        expect(route.path).toEqual(tokens.concat([WNATIVE]))
        expect(route.input).toEqual(tokens[0])
        expect(route.output).toEqual(WNATIVE)
      })
      ...
      describe('Trade', () => {
          ...
          route = new Route(
            [
              new Pair(
                ...
                new TokenAmount(WNATIVE, decimalize(10, WNATIVE.decimals))
              )
            ],
            tokens[1]
          )
          const inputAmount = new TokenAmount(tokens[1], decimalize(1, tokens[1].decimals))
          const expectedOutputAmount = new TokenAmount(WNATIVE, '1663192997082117548')
        ...
        it('TradeType.EXACT_OUTPUT', () => {
          const outputAmount = new TokenAmount(WNATIVE, '1663192997082117548')
              ...
              [
                new Pair(
                  ...
                  new TokenAmount(
                    WNATIVE,
                    JSBI.add(
                      decimalize(10, WNATIVE.decimals),
      ...
      it('TokenAmount', () => {
        const amount = new TokenAmount(WNATIVE, '1234567000000000000000')
